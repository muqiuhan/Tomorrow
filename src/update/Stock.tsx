 ...
 */
export const PercentChangeInfo = (percentChange: string): Either.Either<string, [string, string, string]> => {
  return Fun.pipe(Number.parseFloat(percentChange), (value: number) => {
    if (Number.isNaN(value)) {
      return Either.left(`Failed to parse the percent_change: ${percentChange}`)
    } else {
      return Either.right(
        [
          value > 0 ? 'red' : 'green',
